"""Build C09.
    ...
    python3 lessons/C09/build.py            write C09.ipynb and lesson.md
    python3 lessons/C09/build.py --check    fail if either one is out of date
...
lesson.md(f"""# C09: The lock that deadlocks tomorrow
...
lesson.block("hook")
lesson.md("""Here is a kernel module that cannot hang.
...
lesson.block("tour")
lesson.md(f"""## It is not watching for hangs
...
{lesson.image("the-cycle.svg", "A graph with two boxes. On the left a box labelled lock_a, with the address c1a4e0a0 and the words one lock class under it, and on the right the same for lock_b at c1a4e060. A solid arrow runs from lock_a to lock_b, labelled edge one, recorded when the first thread held lock_a and took lock_b. A dashed arrow runs back from lock_b to lock_a, labelled edge two, about to be recorded because the second thread holds lock_b and is taking lock_a. A note by the dashed arrow says it has not been added and that this is the moment lockdep says no. Underneath, three notes say what an edge means, that a loop means there is an order in which two of these block forever, and that nothing here is blocked because both threads finished.")}
...
lesson.md(f"""## How to read the numbered chain

Skip the drawing at the bottom of a report. Read the numbered entries in the middle. {lesson.claim("C09-05")}.
...
for one in found:
    print()
    print(one.summary())""",
    note="Reads the reader's own log. Empty output is the normal result on a healthy machine.",
)
...
lesson.md(f"""Here is the same point drawn as a timeline, with the two threads of that module separated by a wait.

{lesson.image("never-overlapped.svg", "A timeline running left to right, with an arrow across the top labelled time, and two rows under it. The top row is the first thread: it takes lock_a, then takes lock_b, then releases both and exits. The bottom row is the second thread, and it starts only after the top row has finished, with a labelled gap between them saying the first thread has already exited. The second thread takes lock_b, then reaches for lock_a, and at that point a marker on the timeline says lockdep reports a cycle. A note underneath says the two rows never overlap, so no thread ever waited for another one, and that the report is about the order the locks were taken in rather than about anything that happened at the same time.")}

Testing looks for the row where the two overlap. Lockdep does not need that row to exist.
""")

lesson.block("experiment")
lesson.md(f"""## Build it and load it
...
print(colab.lesson_text("C09", "assets/abba.c").split("#include")[0].strip()[:600])""",
    note="C09-02 settles when the reader builds this and reads the report out of their own dmesg.",
)
...
lesson.md("""Now take it apart. The parser refuses a report that was only half copied rather than filling in the missing part, because a guessed lock ordering is worse than no answer.
""")
...
lesson.code(
    ...
    note="Grading against the reader's own report, or a refusal that explains itself.",
)
